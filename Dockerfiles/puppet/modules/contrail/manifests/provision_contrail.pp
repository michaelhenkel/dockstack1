...
class contrail::provision_contrail (
    ...
    #    $vip_name = $contrail_internal_vip
    #}
    #elsif ($internal_vip) {
    #    $vip_name = $internal_vip
    #}
    #else {
    #    $vip_name = $config_ip
    #}
...
#    }
    ...
    $contrail_exec_provision_control = "python  exec_provision_control.py --api_server_ip \"$vip_name\" --api_server_port 8082 --host_name_list \"$host_name_list_for_shell\" --host_ip_list \"$host_ip_list_for_shell\" --router_asn \"$router_asn\" --mt_options \"$mt_options\" && echo exec-provision-control >> /etc/contrail/contrail_config_exec.out"
   ...
   exec { "provision-external-bgp" :
	command => "python /etc/contrail/contrail_setup_utils/setup_external_bgp.py --bgp_params \"$external_bgp\" --api_server_ip \"$vip_name\" --api_server_port 8082 --router_asn \"$router_asn\" --mt_options \"$mt_options\" && echo provision-external-bgp >> /etc/contrail/contrail_config_exec.out",
