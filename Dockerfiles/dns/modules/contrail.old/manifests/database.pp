...
class contrail::database (
    ...
    $config_ip = $::contrail::params::config_ip,
    $database_ip_list = $::contrail::params::database_ip_list,
#    $internal_vip = $::contrail::params::internal_vip,
    $zookeeper_ip_list = $::contrail::params::zookeeper_ip_list,
...
#) inherits ::contrail::params {
    ...
    # set database_index
    $database_index = inline_template('<%= @database_ip_list.index(@hostname) %>')
    notify { "tmp index: $tmp_index":;}
    notify { "hostname: $hostname":;}
    #if ($tmp_index == nil) {
    #    fail("Host $host_control_ip not found in servers of database roles")
    #}
    notify { "db index1: $database_index":;}

    # set cassandra_seeds list
    if (size($data_base_ip_list) > 1) {
        $cassandra_seeds = difference($database_ip_list, [$hostname])
    }
    else {
        $cassandra_seeds = $database_ip_list
    }
    notify { "zoo list: $zookeeper_ip_list":;}
    notify { "zoo list: $zookeeper_ip_list":;}
