...
class contrail::compute (
    ...
    exec { "add-vnc-config" :
	command => "/bin/bash -c \"python /opt/contrail/utils/provision_vrouter.py --host_name $::hostname --host_ip $host_control_ip --api_server_ip $vip_name --oper add --admin_user $keystone_admin_user --admin_password $keystone_admin_password --admin_tenant_name $keystone_admin_tenant --openstack_ip $openstack_ip && echo add-vnc-config >> /etc/contrail/contrail_compute_exec.out\"",
