...
class contrail::control (
    ...
    $ctrl_fqdn = join([$::hostname,$domain],'.')
    ...
    exec { "provision control" :
        command => "python /opt/contrail/utils/provision_control.py --api_server_ip $vip_name --api_server_port 8082 --host_name $ctrl_fqdn --host_ip $ipaddress --router_asn 64512 --oper add --admin_user $keystone_admin_user --admin_password $keystone_admin_password --admin_tenant $keystone_admin_tenant",
        provider => shell,
    }
